...
devkit_dir = r'data\PaddleDetection\dataset\voc\VOCdevkit\VOC2012'
output_dir = r'data\PaddleDetection\dataset\voc\VOCdevkit\VOC2012'
    ...
    for _, _, files in os.walk(filelist_dir):
        for fname in files:
            ...
            for line in open(fpath):
                ...
                ann_path = annotation_dir + chr(92) + name_prefix + '.xml'
                print(ann_path)
                #img_path = os.path.join(img_dir, name_prefix + '.jpg')
                img_path = img_dir + chr(92) + name_prefix + '.jpg'
                assert os.path.isfile(ann_path), 'file %s not found.' % ann_path
                assert os.path.isfile(img_path), 'file %s not found.' % img_path

                ann_path = annotation_dir[33:] + chr(92) + name_prefix + '.xml'
                img_path = img_dir[33:] + chr(92) + name_prefix + '.jpg'
